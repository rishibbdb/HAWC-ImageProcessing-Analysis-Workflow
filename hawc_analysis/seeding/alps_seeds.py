...
class ALPSSeederBase:
    """ALPS methods: config, model I/O, source-db builders.
    ...
    docstrings, PLUS the config-reading layer below.

    ADAPTED (per explicit human instruction, overriding the original
    two-config design): __init__ now reads the single unified dot-notation
    pipeline config (the same config.yaml DRIPS/core use) via
    core.config.ConfigManager, instead of ALPS's own space-delimited config
    schema (_load_config_value / testalps.py's required_arguments / ...
    sections). Settings that already exist for DRIPS (coordinates, map_tree,
    detector_response, bins, estimator, fit_name, output_dir, no_retries,
    alternate_spectral_models, roi_radius, roi_template_path,
    likelihood_thresholds, diffuse.*) are reused as-is; ALPS-only settings
    live under a new `alps:` section in config.yaml. See backups_pre_config_unify/
    for the original space-delimited-config version of this file.
    ...
    def __init__(self, config):
        """Initialize ALPS from the unified pipeline config.

        Parameters:
        -----------
        config : str or object
            Path to the dot-notation pipeline config YAML (config.yaml), or
            an already-constructed config object exposing `.get(key, default)`
            with dot-notation keys (e.g. core.config.ConfigManager) and
            `.config_file` (the source path). A shared ConfigManager instance
            may be passed here so DRIPS and ALPS run off one loaded config.
        """
        if isinstance(config, (str, Path)):
            from core.config import ConfigManager
            self.config = ConfigManager(str(config))
        else:
            self.config = config
        self.config_path = str(getattr(self.config, 'config_file', config))

        self.source_info_db = pd.DataFrame(columns=['source', 'morphology_type', 'spectrum_type', 'morphology_params', 'spectrum_params'])
        '''Required Params'''
        self.coord_1    = self.config.get('coordinates.ra')
        self.coord_2    = self.config.get('coordinates.dec')
        self.coord_sys  = self.config.get('coordinates.coord_sys')
        self.map_tree   = self.config.get('fitting.map_tree')
        self.det_res    = self.config.get('fitting.detector_response')
        self.bin_list   = self.config.get('fitting.bins')
        self.estimator  = self.config.get('fitting.estimator')
        self.fit_name   = self.config.get('fitting.fit_name')
        self.pixi_path  = self.config.get('alps.pixi_aerie_folder')

        # ALPS keeps its own flat Logs/FitResults/DataMap/Models tree, rooted
        # at output_dir/fit_name (matching DirectoryManager's root_dir convention).
        output_dir = self.config.get('fitting.output_dir')
        self.parent_dir = os.path.join(str(output_dir), str(self.fit_name)) if output_dir and self.fit_name else None
        ...
        log_local_path    = self.config.get('alps.log_local_path', 'Logs')
        self.log_abs_path = os.path.join(self.parent_dir, log_local_path)

        self.logger = ALPSLogger(str(self.log_abs_path), self.config.get('alps.logging_level', 'INFO'))
        ...
        required_arg_name_list = ['fitting.output_dir/fit_name', 'coordinates.ra', 'coordinates.dec', 'coordinates.coord_sys', 'fitting.map_tree', 'fitting.detector_response', 'fitting.bins', 'fitting.estimator', 'fitting.fit_name', 'alps.pixi_aerie_folder']
        ...
        if(len(none_arg_name) > 0):
            for arg in none_arg_name:
                self.logger.error(f'Config key {arg} was not correctly specified in {self.config_path}. Please double check if all required keys are present and correctly defined')

        fit_results_local_path = self.config.get('alps.fit_results_local_path', 'FitResults')
        self.fit_results_abs_path = os.path.join(self.parent_dir, fit_results_local_path)

        raw_sig_map_local_path = self.config.get('alps.raw_sig_map_local_path', 'DataMap')
        self.raw_sig_map_abs_path = os.path.join(self.parent_dir, raw_sig_map_local_path)

        prev_models_local_path = self.config.get('alps.prev_models_local_path', 'Models')
        ...
        self.roi_radius   = float(self.config.get('roi.roi_radius', 5))
        self.minimum_point_source_TS   = float(self.config.get('likelihood_thresholds.point_source_detection', 25))
        self.minimum_morphology_TS_improvement   = float(self.config.get('likelihood_thresholds.extension_test', 16))
        self.minimum_spectral_TS_improvement   = float(self.config.get('likelihood_thresholds.spectrum_test', 16))
        self.point_source_coord_range   = float(self.config.get('alps.point_source_coord_range', 1))
        self.extended_source_coord_range   = float(self.config.get('alps.extended_source_coord_range', 1))
        self.custom_parameter_boundaries_dict   = self.config.get('alps.custom_parameter_boundaries_dict', None)
        self.include_old_diffuse_model   = bool(self.config.get('alps.include_old_diffuse_model', False))
        self.old_diffuse_model_min_l   = self.config.get('alps.old_diffuse_model_min_l', None)
        self.old_diffuse_model_max_l   = self.config.get('alps.old_diffuse_model_max_l', None)
        self.old_diffuse_model_sigma_b   = float(self.config.get('alps.old_diffuse_model_sigma_b', 1))
        self.include_hermes_diffuse_model   = bool(self.config.get('diffuse.use_diffuse_background', False))
        self.hermes_template_path   = self.config.get('diffuse.diffuse_template_path', None)
        self.free_diffuse   = bool(self.config.get('diffuse.free_diffuse_norm', False))
        self.roi_template_path   = self.config.get('roi.roi_template_path', None)
        self.compute_optional_TS   = bool(self.config.get('alps.compute_optional_TS', True))
        self.compute_optional_uncertainties   = bool(self.config.get('alps.compute_optional_uncertainties', True))
        self.use_fast_TS_estimate   = bool(self.config.get('alps.use_fast_TS_estimate', False))
        self.max_param_retries   = int(self.config.get('fitting.no_retries', 10))

        if(self.include_hermes_diffuse_model):
            if(self.hermes_template_path is None):
                self.logger.error(f'Config key diffuse.diffuse_template_path was not correctly specified in {self.config_path} and diffuse.use_diffuse_background is set to True. Please double check if all required keys are present and correctly defined')

        if(self.include_old_diffuse_model):
            required_arg_list = [self.old_diffuse_model_min_l, self.old_diffuse_model_max_l]
            required_arg_name_list = ['alps.old_diffuse_model_min_l', 'alps.old_diffuse_model_max_l']
            none_arg_name = [name for arg, name in zip(required_arg_list, required_arg_name_list) if arg is None]
            if(len(none_arg_name) > 0):
                for arg in none_arg_name:
                    self.logger.error(f'Config key {arg} was not correctly specified in {self.config_path} and alps.include_old_diffuse_model is set to True. Please double check if all required keys are present and correctly defined')

        self.start_from_raw_map   = bool(self.config.get('alps.start_from_raw_map', True))
        self.start_from_existing_yml_file_model   = bool(self.config.get('alps.start_from_existing_yml_file_model', False))
        self.yml_file_model_path   = self.config.get('alps.yml_file_model_path', None)
        self.start_from_existing_model_file_model   = bool(self.config.get('alps.start_from_existing_model_file_model', False))
        self.model_file_model_path   = self.config.get('alps.model_file_model_path', None)
        self.start_from_existing_fits_file_model   = bool(self.config.get('alps.start_from_existing_fits_file_model', False))
        self.fits_file_model_path   = self.config.get('alps.fits_file_model_path', None)
        self.trusted_source_list   = self.config.get('alps.trusted_source_list', None)
        self.trust_all_alterante_morphologies   = bool(self.config.get('alps.trust_all_alterante_morphologies', False))
        self.trust_all_alternate_spectra   = bool(self.config.get('alps.trust_all_alternate_spectra', False))
        self.perform_point_source_adding_phase   = bool(self.config.get('alps.perform_point_source_adding_phase', True))
        self.perform_morpholgy_testing_phase   = bool(self.config.get('alps.perform_morpholgy_testing_phase', True))
        self.alternate_morpholgy_model_list   = self.config.get('alps.alternate_morpholgy_model_list', None)
        self.perform_spectrum_testing_phase   = bool(self.config.get('alps.perform_spectrum_testing_phase', True))
        # NOTE: intentionally NOT reusing fitting.alternate_spectral_models (DRIPS's
        # own field, already populated) -- this phase hit a real bug (see TASKS.md
        # Task 4 / run_alt_hypothesis: baseline re-fit on an all-frozen, zero-free-
        # parameter model). Keep it opt-in via its own key until that's reviewed.
        self.alternate_spectrum_model_list   = self.config.get('alps.alternate_spectrum_model_list', None)
        self.perform_final_fitting_phase   = bool(self.config.get('alps.perform_final_fitting_phase', True))

        if(self.perform_spectrum_testing_phase):
            if(self.alternate_spectrum_model_list is None):
                self.logger.error(f'Config key alps.alternate_spectrum_model_list was not correctly specified in {self.config_path} and alps.perform_spectrum_testing_phase is set to True. Please provide a correctly formatted list of alternate spectrum models to test if you wish to perform this testing phase')
        if(self.perform_morpholgy_testing_phase):
            if(self.alternate_morpholgy_model_list is None):
                self.logger.error(f'Config key alps.alternate_morpholgy_model_list was not correctly specified in {self.config_path} and alps.perform_morpholgy_testing_phase is set to True. Please provide a correctly formatted list of alternate morpholgy models to test if you wish to perform this testing phase')

        if(self.start_from_existing_yml_file_model):
            if(self.yml_file_model_path is None):
                self.logger.error(f'Config key alps.yml_file_model_path was not correctly specified in {self.config_path} and alps.start_from_existing_yml_file_model is set to True. Please provide a correctly formatted path to use this argument')

        if(self.start_from_existing_model_file_model):
            if(self.model_file_model_path is None):
                self.logger.error(f'Config key alps.model_file_model_path was not correctly specified in {self.config_path} and alps.start_from_existing_model_file_model is set to True. Please provide a correctly formatted path to use this argument')

        if(self.start_from_existing_fits_file_model):
            if(self.fits_file_model_path is None):
                self.logger.error(f'Config key alps.fits_file_model_path was not correctly specified in {self.config_path} and alps.start_from_existing_fits_file_model is set to True. Please provide a correctly formatted path to use this argument')
        ...
        if(self.start_from_raw_map ^ self.start_from_existing_yml_file_model ^ self.start_from_existing_model_file_model ^ self.start_from_existing_fits_file_model):
            ...
            self.logger.error(f'Either no starting point or multiple starting points has been specified in {self.config_path}. Please double check and select exactly one starting point.')

        self.make_raw_data_map   = bool(self.config.get('alps.make_raw_data_map', True))
        self.make_model_map   = bool(self.config.get('alps.make_model_map', True))
        self.make_optional_residual_map   = bool(self.config.get('alps.make_optional_residual_map', True))
        self.plot_size_coord_1   = float(self.config.get('alps.plot_size_coord_1', 10))
        self.plot_size_coord_2   = float(self.config.get('alps.plot_size_coord_2', 10))
