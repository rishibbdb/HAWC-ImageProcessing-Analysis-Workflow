...
def run_joint_fit(seeding_output: SeedingOutput, config, logger, directory_manager) -> FitResult:
    ...
    logger.info(f'Running joint fit on DRIPS seed model ({seeding_output.num_sources} sources)')
    result = runner.fit(
        model_file=str(seeding_output.baseline_model_path),
...
def run(seeding_output: SeedingOutput, config, logger, directory_manager, checkpoint) -> SeedingOutput:
    ...
    result = run_joint_fit(seeding_output, config, logger, directory_manager)
