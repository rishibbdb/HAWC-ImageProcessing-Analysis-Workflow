...
_BIN_RE = re.compile(r'^B(\d+)C(\d+)(?:E(\w+))?$')

def bin_sort_key(bin_name: str):
    """B<n>C<m>[E<suffix>] -> (n, m, suffix). Unparseable names sort last."""
    m = _BIN_RE.match(bin_name)
    if not m:
        return (9999, 9999, bin_name)
    b, c, e = m.groups()
    return (int(b), int(c), e or '')
    ...
    try:
        if args.mode == 'hdf5':
            ...
            if existing_fits:
                ...
                logger.info("No existing FITS files found, performing conversion")
            ...
            pairs = sorted(zip(fits_files, bin_names), key=lambda p: bin_sort_key(p[1]))
            fits_files = [f for f, _ in pairs]
            bin_names = [b for _, b in pairs]
