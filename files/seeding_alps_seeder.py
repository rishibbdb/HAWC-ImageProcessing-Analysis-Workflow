...
class ALPSSeeder(ALPSSeederBase, SeedingModule):
    ...
    -----------
    initial_config : str
        Path to ALPS's own config YAML (space-delimited key schema) — drives
        the ALPS control flow (thresholds, phases, paths).
    fit_config_path : str
        Path to the threeMLFit/pipeline config YAML (dot-notation schema) —
        drives the in-process fitter (map_tree, det_res, bins, ROI).
    roi_template : str, optional
        ROI template forwarded to threeMLFit.
    """

    def __init__(self, initial_config: str, fit_config_path: str, roi_template: str = None):
        # Part 1 __init__ reads the ALPS config and sets up dirs + self.logger.
        ALPSSeederBase.__init__(self, initial_config)

        self.fit_config_path = fit_config_path
    ...
    def _residual_hd5_to_fits(self, fit_result) -> Optional[str]:
        ...
        MapGenerator.create_healpix_map(
